...
const Container = () => {
    ...
    const singleReadData = (readTime) => {
         const previousTime = readTime + books;
    ...
    return (
        <div className='container-divide my-6'>
            ...
            <div>
                <div className='mb-4'><h1 className='container-style text-2xl font-bold'>Spend time on read : {books.length}</h1></div>

                <div className='bg-slate-200 ml-6 pb-3'>
                <h1 className='font-bold text-3xl p-5 '>Bookmarked Blogs : {times.length}</h1>
                {
                    times.map(book => <SingleTile book={book} key={book.id}></SingleTile>)
                }
                </div>
            </div>
